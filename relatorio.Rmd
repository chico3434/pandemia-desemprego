...
```{r }
tabela(table(cut(dados$idade, seq(valores_idade$min, valores_idade$max, l = sturges+1))), 'Distribuição por faixa etária')
...
```{r message=FALSE}
fonte_renda <- data.frame(label = row.names(table(fonte)))

ggplot(fonte_renda, aes(x = label, y = table(fonte))) +
...
```{r}
# ideal seria deixar em ordem e fazer a frequência acumulada
tabela(table(dados$renda_antes), 'Distribuição por renda antes da pandemia')
...
```{r message=FALSE}
renda_antes <- data.frame(label = row.names(table(dados$renda_antes)))

ggplot(renda_antes, aes(x = label, y = table(dados$renda_antes))) +
